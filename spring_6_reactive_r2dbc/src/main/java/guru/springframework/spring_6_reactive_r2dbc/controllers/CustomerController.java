...
import guru.springframework.spring_6_reactive_r2dbc.services.CustomerService;
...
public class CustomerController {
    ...
    public Mono<ResponseEntity<Void>> deleteExistingCustomer(@PathVariable("customerId") Integer customerId) {
        return customerService.deleteCustomer(customerId).map(response -> ResponseEntity.noContent().build());
    }
    ...
    public Mono<ResponseEntity<Void>> createNewCustomer(@Validated @RequestBody CustomerDTO customerDTO) {
        return customerService.saveNewCustomer(customerDTO).map(savedDto -> ResponseEntity
